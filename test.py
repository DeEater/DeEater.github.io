...
for k in months_n:
    for i in days:
        try:
            day = list(data["User-data"]["2023"][k][i])
            # print(day)
            for j in day:
                if(data["User-data"]["2023"][k][i][j]["kickboard"] == '"GCOOTER"'):
                    brands[0] += 1
                    brand_name.append("GCOOTER")
                    months[cnt] += 1
                elif(data["User-data"]["2023"][k][i][j]["kickboard"] == '"SWING"'):
                    brands[1] += 1
                    brand_name.append("SWING")
                    months[cnt] += 1
                elif(data["User-data"]["2023"][k][i][j]["kickboard"] == '"BEAM"'):
                    brands[2] += 1
                    brand_name.append("BEAM")
                    months[cnt] += 1
                elif(data["User-data"]["2023"][k][i][j]["kickboard"] == '"SINGSING"'):
                    brands[3] += 1
                    brand_name.append("SINGSING")
                    months[cnt] += 1

                latitudes.append(data["User-data"]["2023"][k][i][j]["latitude"])
                longitudes.append(data["User-data"]["2023"][k][i][j]["longitude"])
...
for k in months_n:
    for i in days:
        try:
            ...
            # print(day)
            for j in day:
                if(data["User-data"]["2023"][choice][i][j]["kickboard"] == '"GCOOTER"'):
                    choice_month[0] += 1
                elif(data["User-data"]["2023"][choice][i][j]["kickboard"] == '"SWING"'):
                    choice_month[1] += 1
                elif(data["User-data"]["2023"][choice][i][j]["kickboard"] == '"BEAM"'):
                    choice_month[2] += 1
                elif(data["User-data"]["2023"][choice][i][j]["kickboard"] == '"SINGSING"'):
                    choice_month[3] += 1
...
plt.title('Reports in 2023', loc='right', pad=5)
...
plt.ylabel("Number of reports")
plt.title('Reports in 2023-' + choice + '   Total : ' + str(sum(choice_month)), loc='right', pad=5)
plt.tight_layout()
# plt.show()
...
for t in autotexts:
    t.set_color("white")
    t.set_fontsize(10)
plt.title('Reports in 2023   Total : ' + str(sum(brands)), loc='center', pad=5)
# plt.show()
